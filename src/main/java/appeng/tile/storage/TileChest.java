...
public class TileChest extends AENetworkPowerTile implements IMEChest, ITerminalHost, IPriorityHost, IConfigManagerHost, IColorableTile, ITickable
{
	...
	private final IItemHandlerModifiable internalInventory = new CombinedInvWrapper(inputInventory, cellInventory);
	...
	private void tryToStoreContents()
	{
		try
		{
			if( !ItemHandlerUtil.isEmpty( this.inputInventory ) )
			{
				...
				final IAEItemStack returns = Platform.poweredInsert( this, cell, AEApi.instance().storage().createItemStack( this.inputInventory.getStackInSlot(0) ),
						this.mySrc );
	...
	private class CellInventoryFilter implements IAEItemFilter
	{
		...
		public boolean allowInsert(IItemHandlerModifiable inv, int slot, ItemStack stack) 
		{
			if( AEApi.instance().registries().cell().getCellInventory( stack, TileChest.this, StorageChannel.ITEMS ) != null )
			{
				return true;
			}
			if( AEApi.instance().registries().cell().getCellInventory( stack, TileChest.this, StorageChannel.FLUIDS ) != null )
			{
				return true;
			}
			return false;
